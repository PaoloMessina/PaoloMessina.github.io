...
CREATE OR REPLACE VIEW public.vw_lista_adi
AS SELECT
              dadi.seq_id_adi,
              dadi.codice_adi,
              dadi.matricola,
              ...
              (select string_agg(tb1.sanzione, '#') from
                  (
                      select dsa1.desc_sanzione as sanzione from d_sanzioni_accessorie dsa1 where dsa1.fk_seq_id_violazione = daviolazioni.seq_id_adi_violazioni
                  ) as tb1) as desc_sanzione_accessoria
   FROM d_adi dadi
            INNER JOIN d_adi_veicoli daveicoli ON dadi.seq_id_adi = daveicoli.seq_id_adi
            INNER JOIN d_colori_veicoli dcv ON daveicoli.fk_seq_id_colori_veicoli = dcv.seq_id_colori_veicoli
            INNER JOIN d_adi_localita dalocalita ON dadi.seq_id_adi = dalocalita.seq_id_adi
            INNER JOIN d_stati_adi dstatiadi ON dadi.fk_seq_id_stati_adi = dstatiadi.seq_id_stati_adi
            INNER JOIN d_comandi_zona dcz ON dadi.fk_seq_id_comandi_zona = dcz.seq_id_comandi_zona
            INNER JOIN d_utenti dutenti ON dadi.fk_seq_id_utenti = dutenti.seq_id_utenti
            INNER JOIN d_adi_violazioni daviolazioni ON dadi.seq_id_adi = daviolazioni.fk_seq_id_adi
